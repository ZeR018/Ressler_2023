...
def num_rossler_together(x1_n, y1_n, z1_n, h, a, b, c, w1, d, numsteps):
    def d(x1_n, x2_n, y1_n, y2_n):
        d_val = 0
        if (pow(x1_n - x2_n, 2) + pow(y1_n - y2_n, 2) < 9):
            d_val = 0.2
    ...
    for i in range(5):
        for k in range(numsteps-1):
            d_fin = 0
            for j in range(5):
                d_fin = d_fin + d(x1_n1[i][k], x1_n1[j][k], y1_n1[i][k], y1_n1[j][k]) * (y1_n1[j][k] - y1_n1[i][k])
            
            dx_1 = ((-w1[i]) * y1_n1[i][k] - z1_n1[i][k])
            dy_1 = (w1[i] * x1_n1[i][k] + a * y1_n1[i][k]) + d_fin
...
def visualize(x1, y1, z1, t, freq_1, time_frame, steps_t, steps_fr, N):
    ...
    # when_to_track = round((steps_t/10)*2)
    # when_to_track = 0

    # x1_val_track = np.random.rand(5, steps_t)
    # y1_val_track = np.random.rand(5, steps_t)
...
def main():
    ...
    for i in range(N_of_agents):
        ...
        # random start point
        x1[i][0] = np.random.uniform(-15, 15)
        y1[i][0] = np.random.uniform(-15, 15)
        z1[i][0] = 0
    ...
    x1, y1, z1 = num_rossler_together(x1, y1, z1, h, a, b, c, w1, d, numsteps)
