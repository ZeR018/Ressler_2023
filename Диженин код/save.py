...
def num_rossler(x1_n, y1_n, z1_n, h, a, b, c, w1, d_it):
...
def num_rossler_freq(x1_n, y1_n, z1_n, h, a, b, c, w1, d_it):
    def d(x1_n, x2_n, y1_n, y2_n):
        d_val = 0
        if ((x1_n - x2_n)**2 + (y1_n - y2_n)**2 < 3**2):
            d_val = 0.2
...
def visualize(x1, y1, z1, t, freq_1, time_frame, steps_t, steps_fr, N):
    ...
    def update_all(i):
        ...
        return line1, line1_track, line3_1_x, line3_1_y, line3_1_z, point, point_track1, point3_1_x, point3_1_y, point3_1_z
    ...
    anim = FuncAnimation(fig, update_all, frames = np.size(x1[0]), interval = 0, blit = True)
...
def main():
    ...
    for i in range(N_of_agents):
        w1=np.random.uniform(0.93,1.07) # random in the range of (0,93 .. 1,07)
        ...
        for k in range(numsteps-1):
            [x1[i][k+1], y1[i][k+1], z1[i][k+1]] = num_rossler(x1[i][k], y1[i][k], z1[i][k], t[k+1] - t[k], a, b, c, w1, d[k])

        for k in range(d_numsteps-1):
            [freq_1[i][k+1], phase_diff_1[i][k+1]] = num_rossler_freq(x1[i][k], y1[i][k], z1[i][k], t[k+1]-t[k], a, b, c, w1, d[k])
